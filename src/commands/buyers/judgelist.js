...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('judgelist')
        .setDescription('liste des roles juges'),
    permissions: ['buyer'],
    async execute(interaction, client) {
        try {
            const guildId = interaction.guild.id;
            const rows = db.prepare('SELECT role_id FROM judges WHERE guild_id = ?').all(guildId);

            if (rows.length === 0) {
                const container = new ContainerBuilder()
                    .setAccentColor(config.embedColor)
                    .addTextDisplayComponents((td) => td.setContent(`> ${config.emojis.error} aucun juge config`));
            ...
            const generateComponents = (page) => {
                ...
                const rolesList = chunk.map(r => `- ${parser.role(r.role_id)} (${r.role_id})`).join('\n');

                const container = new ContainerBuilder()
                    .setAccentColor(config.embedColor)
                    .addSeparatorComponents((s) => s)
                    .addTextDisplayComponents((td) => td.setContent(`## Liste Juge`))
                    .addTextDisplayComponents((td) => td.setContent(`> Vous pouvez maintenant consulter la liste des juges\n> Nombre de role étant juge : ${total}`))
                    .addSeparatorComponents((s) => s)
                    .addTextDisplayComponents((td) => td.setContent(rolesList))
                    .addSeparatorComponents((s) => s);

                container.addSectionComponents((section) => {
                    section.addTextDisplayComponents((td) => td.setContent(`Page **${page + 1}**/**${pagesNum}**`));
                    section.setButtonAccessory((btn) =>
                        btn.setCustomId('next_page_judge')
                            .setLabel('Suivant')
            ...
            console.error('probleme dans judgelist', err);
            const container = new ContainerBuilder()
                .setAccentColor(config.embedColor)
