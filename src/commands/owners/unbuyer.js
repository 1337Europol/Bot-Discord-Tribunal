...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('unbuyer')
        .setDescription('retirer un buyer')
    ...
    async execute(interaction, client) {
        ...
        if (!exists) {
            const container = new ContainerBuilder()
                .setAccentColor(config.embedColor)
                .addTextDisplayComponents(td => td.setContent(`> ${config.emojis.error} <@${userId}> est pas buyer`));
            return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true });
        }

        db.prepare("DELETE FROM buyers WHERE guild_id = ? AND id = ?").run(interaction.guild.id, userId);

        const container = new ContainerBuilder()
            .setAccentColor(config.embedColor)
            .addTextDisplayComponents(td => td.setContent(`> ${config.emojis.success} <@${userId}> plus buyer`));
