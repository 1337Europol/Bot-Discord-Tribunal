...
module.exports = {
    ...
    async execute(interaction, client) {
        try {
            ...
            const rows = db.prepare('SELECT id FROM owners WHERE guild_id = ?').all(guildId);

            if (rows.length === 0) {
                const container = new ContainerBuilder()
                    .setAccentColor(config.embedColor)
                    .addTextDisplayComponents((td) => td.setContent(`> ${config.emojis.error} personne est owner`));
            ...
            const generateComponents = (page) => {
                ...
                const container = new ContainerBuilder()
                    ...
                    .addTextDisplayComponents((td) => td.setContent(`## Liste Owner`))
                    .addTextDisplayComponents((td) => td.setContent(`> Vous pouvez maintenant consulter la liste des owners\n> Nombre de membre étant owner : ${total}`))
                    .addSeparatorComponents((s) => s)
                ...
                container.addSectionComponents((section) => {
                    ...
                    section.setButtonAccessory((btn) =>
                        btn.setCustomId('next_page_owner')
                            .setLabel('Suivant')
            ...
            console.error('probleme dans ownerlist', err);
            const container = new ContainerBuilder()
                .setAccentColor(config.embedColor)
